## Problem: These are the assignment problems taken from the Java Section of Coding Bat but modified for Python. 
...
def groupSum5(start, nums, target):
    ...
    elif nums[start] == 1 and nums[start - 1] == 5:
...
def groupSumClump(start, nums, target):
    ...
    if start >= len(nums):
        ...
        while count < len(nums) and nums[start] == nums[count]:
            sum += nums[start]
            count += 1
        return groupSumClump(count, nums, target - sum) or groupSumClump(count, nums, target)
...
def splitArrayHelper(index, nums, A, B):
    if index >= len(nums):
        return A == B
    else:
        return splitArrayHelper(index + 1, nums, A + nums[index], B) or splitArrayHelper(index + 1, nums, A, B + nums[index])
...
def splitOdd10Helper(index, nums, A, B):
    if index >= len(nums):
        return A % 10 == 0 and B % 2 == 1
    else:
        return splitOdd10Helper(index + 1, nums, A + nums[index], B) or splitOdd10Helper(index + 1, nums, A, B + nums[index])
...
def split53Helper(index, nums, A, B):
    if index >= len(nums):
        return A == B
    elif nums[index] % 5 == 0:
        return split53Helper(index + 1, nums, A + nums[index], B)
    elif nums[index] % 3 == 0:
        return split53Helper(index + 1, nums, A, B + nums[index])
    else:
        return split53Helper(index + 1, nums, A + nums[index], B) or split53Helper(index + 1, nums, A, B + nums[index])
