## Problem: These problems are taken from the Java Section of Coding Bat but modified for Python.
...
def strDist(str, sub):
    ...
    # Start is sub
        ...
        # And end is sub
        if str[-len(sub): -1] + str[-1]  == sub:
            return len(str)
        else:
            return strDist(str[:-1], sub)

    # End is sub
    elif str[-len(sub): -1] + str[-1] == sub:
        return strDist(str[1:], sub)
    # Sub not on either end.
    else:
